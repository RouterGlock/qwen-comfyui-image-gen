...
| Tool call hangs, then times out after 5 minutes | ComfyUI isn't reachable at `COMFYUI_URL`, or the model is still loading on first run. Edit/reference generations take noticeably longer than plain generation (the extra reference-image conditioning adds real time on top of sampling) — on slower hardware even 5 minutes may not be enough; raise the `timeoutMs` default in `waitForImage` in `src/toolsProvider.ts` if so. |
